...
import { Users, Shield, BarChart3, Settings } from 'lucide-vue-next';
...
const canAccess = (permission: string | null) => {
    ...
    return user.value?.permissions?.includes(permission) || 
           user.value?.roles?.some((role: any) => role.permissions?.includes(permission));
};
                ...
                <CardContent>
                    <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        <div v-for="action in availableActions" :key="action.title" 
                             class="flex items-center space-x-4 rounded-lg border p-4 hover:bg-accent hover:text-accent-foreground transition-colors">
                            <component :is="action.icon" class="h-8 w-8 text-primary" />
                            <div class="flex-1 space-y-1">
                                <h4 class="text-sm font-medium leading-none">{{ action.title }}</h4>
                                <p class="text-sm text-muted-foreground">{{ action.description }}</p>
                            </div>
                            <Button :as="Link" :href="action.href" size="sm" variant="outline">
                                Go
                            </Button>
                        </div>
                    </div>
